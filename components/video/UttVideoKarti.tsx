...
export function UttVideoKarti({ video, onVideoClick, onBegeni, onFavori }: VideoEtkilesimProps) {
      ...
      <div className="p-2.5">
        <div className="flex items-start justify-between gap-1.5">
          ...
          <div className="flex flex-shrink-0 items-center gap-0.5">
            <button type="button" onClick={(event) => onBegeni(event, video.yayin_id)} aria-label="Beğen" className={`rounded-full p-0.5 transition-colors ${video.begeni_mi ? "text-red-500" : "text-gray-400 hover:text-gray-600"}`}>
              <svg className="h-3.5 w-3.5" fill={video.begeni_mi ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
            </button>
            <span className="text-[10px] text-gray-500">{video.begeni_sayisi}</span>
            <button type="button" onClick={(event) => onFavori(event, video.yayin_id)} aria-label="Favoriye ekle" className={`rounded-full p-0.5 transition-colors ${video.favori_mi ? "text-yellow-500" : "text-gray-400 hover:text-gray-600"}`}>
              <svg className="h-3.5 w-3.5" fill={video.favori_mi ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
            </button>
...
export function UttKayanVideoRafi<T extends UttVideo>({ baslik, videolar, onVideoClick, onBegeni, onFavori, kartAlti }: VideoEtkilesimHandlerlari & { baslik: ReactNode; videolar: T[]; kartAlti?: (video: T) => ReactNode }) {
    ...
    <div className="mb-6">
      ...
      <div className="group relative">
        ...
        <div ref={raf} className="-mx-1 flex snap-x gap-2 overflow-x-auto px-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {videolar.map((video) => <div key={video.yayin_id} className="flex w-40 flex-shrink-0 snap-start flex-col gap-1 sm:w-44 md:w-52"><UttVideoKarti video={video} onVideoClick={onVideoClick} onBegeni={onBegeni} onFavori={onFavori} />{kartAlti?.(video)}</div>)}
        </div>
