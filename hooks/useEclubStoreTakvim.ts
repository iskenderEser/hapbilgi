...
import {
  ...
  type EclubTakvimDurumu,
} from "@/lib/eclub/store/takvim";

export interface UseEclubStoreTakvimSecenekler {
  aktif?: boolean;
}

export interface UseEclubStoreTakvimSonuc {
  takvim: EclubTakvimDurumu | null;
  acik: boolean;
  yukleniyor: boolean;
  yenile: () => Promise<void>;
}
...
/**
 * E-Club Store aylık sipariş takvimini sunucu zamanına bağlı olarak takip eden ortak hook.
 *
 * SÖZLEŞME:
 * - Tarayıcının cihaz saatini (new Date()) açılış/kapanış kararının kaynağı olarak KULLANMAZ.
 * - Sunucunun /eclub/store/api?tip=takvim yanıtındaki simdiIso zamanı çapa (anchor) kabul edilir.
 * - Çapa sonrasında geçen süre, monotonic performance.now() ile hesaplanır; cihaz saatinin
 *   yanlış olması veya kayması durumu bozmaz.
 ...
 */
export function useEclubStoreTakvim({
  aktif = true,
}: UseEclubStoreTakvimSecenekler = {}): UseEclubStoreTakvimSonuc {
  const [takvim, setTakvim] = useState<EclubTakvimDurumu | null>(null);
  ...
  const sunucudanCek = useCallback(async () => {
    if (!aktif) return;
    try {
      const res = await fetch("/eclub/store/api?tip=takvim", { cache: "no-store" });
      if (!res.ok) return;
      ...
      if (data?.takvim?.simdiIso) {
        ...
        const guncel = eclubStoreTakvimDurumu(new Date(serverMs));
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const yeniDurum = eclubStoreTakvimDurumu(currentServerTime);
