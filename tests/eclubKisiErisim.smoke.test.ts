...
test("sınır: pasif veya E-Club'ı kapalı firma dış müşteriye modül açmaz (Eczanem dahil)", () => {
  const sonuc = eclubKisiModulDurumu([
  ...
  assert.deepEqual(sonuc, { eclub_aktif: false, eclub_store_aktif: false, eczanem_aktif: false });
});
