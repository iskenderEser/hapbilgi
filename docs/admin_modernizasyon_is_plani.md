...
**K-A7 — Toplu yükleme UPSERT modeli + telefon kimlik çekirdeği (İskender kararları, 18.07.2026).** Kullanıcı takibi kalıcı `kullanici_id` üzerinden; Excel eşleştirme anahtarları e-posta + telefon. Telefon kimlik çekirdeğine girdi (telefonsuz satır yüklenmez; `05XXXXXXXXX` normalize + benzersiz index; mevcut kayıtlara sahte `0532000XXXX` backfill). İkinci liste mevcut kayıtları günceller (şifre ASLA ezilmez; yeni kullanıcıda zorunlu), listede olmayana dokunulmaz, çakışma/mükerrer görünür satır hatasıdır. T-1: aktif firmaya eksikli yükleme = yükle + uyarı. T-7: eksikli kullanıcı PASİF doğar, eksiği kapanınca otomatik aktifleşir (bilinçli pasif korunur); firma aktifleştirme adminde kalır (kilit kalkınca tek tık). Ayrıntı: admin_guncellestirme.md (upsert aşaması kaydı).

**K-A8 — Toplu yüklemede organizasyon dosyadan kurulur (İskender kararı, 18.07.2026; fiziksel test bulgusu).** Dosyada adı geçen ama firmada olmayan takım/bölge "eksik bilgili" değil, KURULUM PLANIDIR: önizleme "Bu yüklemeyle oluşturulacak: N takım, M bölge" kırılımını ve satır bazında oluşturulacakları gösterir (yazım hatasına karşı göz onayı); kaydette önce takım/bölge açılır (`organizasyonKurulumPlani` — saf plan, yazım rotada), satırlar sonra bilinen akışla işlenir — kayıt tek dosyayla tamamlanır ve eksik kalmadığı için kullanıcılar aktif doğar (T-7). Sınırlar: takımsız bölge OLUŞTURULMAZ (uydurma bağ yok — satır eksik kalır); rol gereği takım taşımayan satırlardaki değerler ("Merkez" vb.) yok sayılır; tekil rota ve Organizasyon sekmesi değişmez. Yan kazanım: takım/bölge ad eşleşmesi tr-TR katlamasına alındı (`adKatla` — düz toLowerCase "İzmir/izmir" eşleşmesini bozuyordu; smoke bulgusu).
...
**UPSERT AŞAMASI — KOD TARAFI BİTTİ (18.07.2026); kapanış fiziksel test bekliyor.** K-A7 kararlarının uygulaması, dört commit: Adım 1 şema (telefon kolonu + benzersiz index + backfill — SQL İskender), Adım 2 telefon kimlik çekirdeği (telefonNormalize, şablon/başlık/tekli form/liste), Adım 3 upsert (satirUpsertPlani: e-posta+telefon eşleşme, önizlemede İşlem sütunu, auth telafili güncelleme, tekil telefon ekleme; ham-rol-DB açığı kapandı → T-5), T-1/T-7 (pasif doğum + otomatik aktifleşme + amber vurgu). Her adım üçlü doğrulama + saf-fonksiyon smoke ile kapandı. **Kapanış koşulu:** İskender'in fiziksel testi — eksikli liste → düzeltilmiş liste (upsert) → otomatik aktifleşme → firma kilidi/aktivasyon. M3'ün bekleyen 11 kişilik testi bu turla birleşir.

**T-SERİSİ — KOD TARAFI BİTTİ (18.07.2026).** 18.07 soğuk taramasının 7 maddesinin tamamı kapatıldı (T-1/T-5/T-7 karar + upsert commit'leri; T-2 FirmaSidebar eksik rozeti, T-3 bordo görsel dili, T-4 PUT rolCoz, T-6 .xls kabulü — dört ayrı commit). Kayıt ve kapanış koşulları: admin_guncellestirme.md (guvenilir_olmayan_refactoring_plani.md silindi, yerine o belge geçti).
