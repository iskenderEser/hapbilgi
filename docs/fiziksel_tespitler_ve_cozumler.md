...
**Durum:** KOD TARAFI BİTTİ — İskender'in fiziksel doğrulaması sonrası ÇÖZÜLDÜ yazılacak. Ara sonuç (18.07): görsel test başarılı; e-posta ayağı sahte @test.com adresleriyle denenemedi, İskender gerçek e-postayla deneyip sonucu paylaşacak. Doğrulama adımları: (1) push sonrası canlıda "Şifremi unuttum" → e-posta → bağlantı → yeni şifre → yeni şifreyle giriş; (2) işaretsiz "Beni hatırla" ile giriş → tarayıcıyı tamamen kapat-aç → login'e düşmeli. Ön koşul: Supabase panelinde Authentication → URL Configuration'da site adresi + `/sifre-yenile` Redirect URL listesinde olmalı (İskender kontrol edecek).
