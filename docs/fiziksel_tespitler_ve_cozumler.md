...
**Ek karar (İskender, 19.07):** Hazır akışı yalnız PM ailesi değil, TÜM üretici roller aynı şekilde kullanır. Kontrolde görüldü: talep açma tüm üreticilere açıkken yürütme uçları (vezne, adres yazma, onay) `pm/jr_pm/kd_pm`'e kilitliydi — PM dışı üretici talep açabiliyor ama video yükleyemiyordu (vezne rol reddi), talep yarım kalıyordu. Düzeltme: üç uç + iptal ucu `URETICI_ROLLER`'e açıldı; onay yetkisi rolde değil üreticiliktedir (uretici_id şartı sürer — herkes yalnız kendi talebini yürütür). Form onay modalındaki eski akış cümleleri ("IU videoyu Bunny.net'e yükleyecek…") yeni akışa göre düzeltildi. Açık uç: "Hazır Soru Setim Var" TEK BAŞINA (video normal akışta) seçildiğinde vaat edilen IU işleme mekanizması soru seti ekranında yok görünüyor — ayrı inceleme bekliyor, İskender kararına bırakıldı.

**Durum:** KOD BİTTİ — fiziksel teyit İskender'de (hazır video + hazır soru setli talep: onay sonrası yayın bekleyenlerine düşmeli, izlemede video başı sayı kadar soru çıkmalı; artık PM dışı bir üretici rolle de denenebilir).
