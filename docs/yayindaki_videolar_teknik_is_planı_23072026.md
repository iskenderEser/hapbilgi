...
- **23.07 — Ek (ana sayfa UX): üretici "İçerik Takibi" tablosuna boş-durum placeholder'ı.** Talebi olmayan üretici (ör. Medikal Müdür Sema) ana sayfada tabloyu "kırık/eksik" görmesin diye: **hiç talep yoksa** başlıklar + **soluk (opacity 0.5, italik) örnek satır** (Ürün adı · Teknik adı · UTT · Senaryo · İnceleme Bekliyor · —) + *"Henüz üretimin yok. İlk talebini açtığında üretim akışın burada görünecek."* gösterilir; **filtre yüzünden boşsa** eski *"Bu kategoride içerik bulunmuyor."* mesajı korunur. Masaüstü (satır) ve mobil (kart) ayrı. Yalnız görünüm — veri/API/DB dokunulmadı ([`UreticiAnaSayfa.tsx`](../components/ana-sayfa/UreticiAnaSayfa.tsx)).
  - **Önemli tespit:** [`getUreticiAnaSayfaVeri`](../lib/utils/anaSayfa/uretici.ts) zaten **role-bağımsız** — talepleri `uretici_id = giriş yapan kullanıcı` ile çeker. Yani "İçerik Takibi" tablosu **tüm üretici rollerinde** (med_md, egt_*, ik_* dahil) kendi üretimini gösterecek şekilde çalışıyordu; role göre yeni tablo/kod yazmaya gerek yoktu. Sema'da boş görünmesinin sebebi kod değil **veri** (talep yokluğu). Bu adımda yalnız boş görünüm iyileştirildi.
- **23.07 — Ek (ana sayfa sadeleştirme): üretici ana sayfasından alt "Videolar" bölümü kaldırıldı.** Videolar artık "Yayındaki Videolar" sekmesinde olduğu için üretici ana sayfasındaki **alt video bölümü** + ölü kalan tam ekran oynatıcı/`aktifVideo` state + kullanılmayan importlar (`VideoBolumu`/`VideoOynatici`/`AnaSayfaVideo`) + `PMVeri.videolar` kaldırıldı. Diğer sayfalar (yönetici/TM/BM/UTT ana sayfaları, Yayındaki Videolar sekmesi) ve `/ana-sayfa/api` **dokunulmadı**. **Erişim kaybı yok** — 13 üretici rolün tamamı `YAYINDAKI_VIDEO_GORENLER`'de, videolarını sekmede görüyor. tsc/denetim/lint temiz.
