...
*Sürüm: Tamamlandı — Bölüm 1–6. Temmuz 2026. Güncellemeler (29.07.2026): **Üretim hattının yeniden kurulması işlendi** — değişen kesitler: §2.6 (arayüz merkezleri: pill + liste), §2.7 (Bunny doğrudan yükleme/vezne modeli, dikey video ve oranlı çerçeve), §2.8 (dört üretim varyantı, zincirin talebe doğrudan bağlanması, durum sözlüğü, firma-önekli talep kimliği, talep merkezli `/talepler-v2`), §3.1 (üretim ekranlarının giriş disiplini, Yayındaki Videolar), §6.1 (talep ve yayın silme), §6.4 (fiziksel test turları ve zincir kırıkları kapananlara işlendi). Kaynak belgeler: `talepler_v2_is_plani.md`. Önceki güncellemeler (18.07.2026): **Admin modernizasyonu işlendi** (M0–M3 + M2 bilgi mimarisi + upsert aşaması + T-serisi; değişen kesitler: §2.2, §2.4, §6.1 yeniden yazıldı, §6.2, §6.4) ve **Web Push kod üretimi tamam** (P0–P7 + K-P12, 16–18.07; "push bekletiliyor" durumu artık yalnız canlıya alma içindir). Daha önceki güncellemeler: Eczanem üçüncü müşteri katmanı (12.07.2026 — yeni Bölüm 5 + değişen kesitler: §1.2, §2.2–2.4, §2.8, §6.4; U0–U9 kod üretimi tamam, commit'lerde), Tekrar Gönderim Modeli + Ekstra İzlediklerim (10.07.2026). Fiziksel insan testleri (U10/U11 + Tekrar Gönderim U10 + Ekstra U7 + admin toplu yükleme), RLS ve diğer deploy-öncesi mecburi işler §6.4'te teknik borç olarak listelidir.*
...
- [7. HapBilgi Geliştirme Adımları (Modül ve Tarih)](#7-hapbilgi-geliştirme-adımları-modül-ve-tarih)
  ...
  - Talep merkezli üretim — `/talepler-v2` (28–29.07.2026)
...
**Talep merkezli üretim — `/talepler-v2` (28.07.2026).** Aşamaların ayrı sayfalarda yaşadığı düzene paralel, ikinci bir rota kuruldu: kullanıcı talepten çıkmadan tüm üretim sürecini yönetir. Temel ilkesi "aşama ayrı bir *yer* değil, talebin içindeki *adım*"dır. Sayfa her talepte sabit beş adımlık dikey bir şerit çizer (Talep → Senaryo → Video → Soru Seti → Yayın); varyant adım **sayısını** değil **halini** değiştirir — o kolda üretilmeyecek adımlar kapalı gösterilir ve işleme kapatılır, böylece kullanıcı hangi yöntemi seçerse seçsin sürecin tamamını görür. Şeridin kaynağı yeni bir sorgu değildir: `v_uretici_icerik_takip` satırı zaten her aşamanın durumunu ayrı taşır, saf çözücü (`lib/utils/uretimSeridi.ts`) beş adımın halini o satırdan türetir ve kaskadı yeniden yazmaz — `asamaCoz()` çağrılır, böylece ana sayfa, Talepler ve şerit aynı cevabı verir. Rota mevcut yapıya hiç dokunmaz: yazma uçları ve ortak katman import-only, yeni tablo/view/SQL yoktur. Faz 1'de yalnız üretici rollere açıktır.
...
*Kaynak: `talepler_v2_is_plani.md`.* Senaryo, video ve soru seti aşamaları ayrı sayfalarda yaşıyordu; kullanıcı bir talebin nerede olduğunu görmek için ekran değiştiriyordu. Bunları tek sayfada birleştiren ikinci bir rota, mevcut yapıya **hiç dokunmadan** paralel kuruldu — gerekçe açıktı: mevcut yapı çalışıyor ve üretim sürüyor, aksaklık çıkarsa işleyen yapı kaybedilmemeli. Yazma uçları ve ortak katman import-only kaldı, yeni tablo/view/SQL üretilmedi. Sayfa her talepte sabit beş adımlık dikey şerit çizer; varyant adım sayısını değil halini değiştirir. Şeridin kaynağı yeni bir sorgu değildir: `v_uretici_icerik_takip` satırı zaten her aşamanın durumunu ayrı taşır, saf çözücü beş adımın halini o satırdan türetir ve kaskadı yeniden yazmaz. A-1'den A-11'e on bir adımda koşuldu, A-12'de dört varyant (V1–V4) fiziksel testten geçti (`da7553b`). Test dokuz bulgu üretti; ikisi aynı gün kapatıldı — form mesajları ekranda hiç görünmüyordu (mesaj kapsayıcısı yanlış hook'a bağlıydı) ve şerit metinleri düzeltildi. Aynı turda dört yerde ortak yapıya geçildi: soru seti ayarları, soru şıkları (pill merkezi), video yükleme alanı ve tarih biçimi. Yayın Listesi'nin kapsamı da daraltıldı — yalnız üretimi bitenler görünür, aynı talep artık iki ekranda birden çıkmaz (`4fd3bec`).
