...
| `rol-tek-kaynak` | "Kim görebilir" rol kararının elle yazılmaması — tek kaynak `lib/utils/roller.ts` adlandırılmış grupları; içinde ≥2 hard-code rol string'i olan diziyi reddeder (tekil karşılaştırma / `hedef_rol` / tip-union kapsam dışı). B-09 temizliğiyle ratchet baseline'ı 50→0'a indi; kural tam sıkıdır (30.07.2026). |
