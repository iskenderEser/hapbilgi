...
- **A1** — Yeni saf yardımcı `lib/video/anaSayfaRaflari.ts`: girdi utt-görünür video listesi; çıktı `{ tumuRafi: Video[] (random ≤5), departmanRaflari: {tur, videolar: Video[]}[] (her tur 5-üstünlük) }`. Algoritma **tek yerde**, saf ve test edilebilir; random tohumu parametreyle (yükleme başına sabit). Tek-kaynak ilkesi.
