...
**Kapananlar — üretici ana sayfa performansı (27.07.2026).**
- **Üretici ana sayfa N+1 deseni** (§2.6): `lib/utils/anaSayfa/uretici.ts`'teki talep başına sıralı sorgu zinciri (1+7N — altı talepli bir üreticide 43 ardışık gidiş-geliş, `/ana-sayfa/api` 2.93 sn) kapandı. Üretim zinciri `v_uretici_icerik_takip` view'ine (LATERAL) taşındı; route iki paralel sorgu yapar (talepler ∥ view), maliyet talep sayısından bağımsız sabittir — 43 sorgu → 2. Yöntem seçimi kurala uygundur: sayfa hesaplamıyor listeliyor, dolayısıyla RPC değil view (emsal: Yayındaki Videolar → `v_yayin_detay`). View yalnız **ham durum** döndürür; durum metni/rengi `mesaj.ts` sözlüğünde kalır, aşama seçimi saf `asamaCoz()` fonksiyonundadır. Kolonlar açık seçilir (`select("*")` değil) ki `denetim` kolon değişimini yakalasın. DDL + GRANT: `scripts/sql/v_uretici_icerik_takip.sql` (canlıda koşuldu — yeni view otomatik yetki almaz, service_role SELECT alamazsa sayfa "permission denied for view" ile boşalır); zincir kolonlarına indeksler eklendi. Kaskad davranışı birebir; tsc + denetim + lint:mimari temiz (commit `9fb6839`).
