...
*Sürüm: Tamamlandı — Bölüm 1–6. Temmuz 2026. Güncellemeler (12.07.2026): **Eczanem üçüncü müşteri katmanı işlendi** (yeni Bölüm 5 + değişen kesitler: §1.2, §2.2–2.4, §2.8, §6.4) — U0–U9 kod üretimi tamam, commit'lerde. Önceki güncelleme (10.07.2026): Tekrar Gönderim Modeli + Ekstra İzlediklerim. Push bilinçli bekletiliyor: fiziksel insan testleri (U10/U11 + Tekrar Gönderim U10 + Ekstra U7), RLS ve diğer deploy-öncesi mecburi işler §6.4'te teknik borç olarak listelidir.*
...
**Test verisi temizliği.** `app/admin/test-temizlik/` ve karşılığı `admin/api/test-verileri-sil`, geliştirme/test ortamında oluşturulan verinin temizlenmesi içindir. Bu endpoint proxy'de bilinçli olarak girişsiz bırakılmıştır (§2.4) ve deploy öncesi tamamen kaldırılması gereken bir araçtır (§6.4).
...
- **RLS eksikliği** (§2.4): Tabloların büyük çoğunluğunda Row Level Security kapalı; şu an tek savunma katmanı API/proxy seviyesindeki manuel rol kontrolüdür (service_role RLS'i bypass eder). Yaklaşık 30 tabloya (`yayin_tekrar_kayitlari` dahil) **+ 11 yeni `eczanem_*` tablosuna** RLS eklenmesi ve hangi işlemlerin service_role, hangilerinin anon+RLS ile yapılacağının netleştirilmesi gerekiyor. (Eczanem K-E7 kararı fiilen bu genel işe katlandı: tablolar RLS'siz doğdu, service_role'e açık GRANT'li, anon/authenticated'a bilinçli kapalı.)
- **Test endpoint'lerinin kaldırılması** (§2.4, §6.1): `admin/api/test-verileri-sil` ve `app/admin/test-temizlik/` bilinçli olarak girişsiz bırakılmış test araçlarıdır; deploy öncesi tamamen kaldırılmalı.
...
- **K-E1 — SMS sağlayıcısı (tek gerçek blokör)**: Sağlayıcı seçimi maliyet/sözleşme kararıdır; entegrasyon noktası hazırdır (`lib/sms/gonderici.ts` provider-agnostik, canlı-dışı ortamlar K-E8 test moduyla sağlayıcısız çalışır). Karar gelmeden Eczanem canlıya çıkamaz.
