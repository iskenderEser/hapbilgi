...
- **23.07 — Admin kullanıcı telefon kuralı düzeltildi (10 hane) + mevcut veri normalize edildi.** *(Üretim süreci dışı — admin/kullanıcı; bu oturumda çıktığı için buraya kaydedildi.)* **Belirti:** admin tekil/toplu kullanıcı kaydında geçerli TR cep no (ör. `5420000033`) *"Geçersiz telefon… 11 hane"* hatasıyla reddediliyordu. **Kök neden:** [`kullaniciDogrulama.ts telefonNormalize`](lib/admin/kullaniciDogrulama.ts) regex `^5\d{10}$` = **11 hane** ister; "ilk hane 5 + 0 ile başlamaz + 11 hane" kuralı gerçek TR numarasında imkânsız (0'sız 5-li numara 10 hanedir). Sadece hatalı-uzun numarayı kabul edip doğru numarayı hep reddediyordu — admin'den kullanıcı kaydını fiilen bloke eden hata. **Çözüm (İskender kuralı, 23.07):** 10 hane, ilk hane **5 zorunlu** (0 dahil başka rakamla başlayamaz), gösterim 3-3-4 `542 000 0000`. Doğrulama `^5\d{9}$`; tekil+toplu tek fonksiyondan geçtiği için **ikisi de** düzeldi. Yeni tek biçim kaynağı [`lib/admin/telefonBicim.ts`](lib/admin/telefonBicim.ts) (rakam ayıklama + 3-3-4 biçimleme); giriş alanları ([`TekilGirisFormu`](app/admin/_components/TekilGirisFormu.tsx), [`KullaniciListesi`](app/admin/_components/KullaniciListesi.tsx) inline) kuralı yazarken uygular (5-dışı baş anında silinir). E-Club telefonu (ayrı 11-hane kuralı, farklı alan) dokunulmadı. **Mevcut veri (SQL → İskender koştu):** 44 firma kullanıcısından 43'ü `0` önekli (`0532…/0542…`) → baştaki `0` kırpılıp 10 haneye indirildi; 1 kayıt eski hatalı kuralın ürünü (11 hane 5-li `54200000022`) elle `5420000022`'ye düzeltildi. Son durum: tüm firma kullanıcı telefonları tek formatta (`hala_bozuk=0`). tsc/denetim/lint temiz.
- **23.07 — Hazır video + hazır soru seti varyantı fiziksel test (başarıyla geçildi) + talep formu küçük iyileştirmeleri (üretici=PM, tüketici=UTT).** Varyant (İU'ya iş düşmez — PM doğrudan yayına, UTT tüketir) uçtan uca test edildi ve geçti. Test sırasında iki küçük form iyileştirmesi yapıldı:
  - **Bilgi kutusu metni sadeleştirildi** (hazır video + hazır soru seti birlikte seçiliyken): eski teknik detay (senaryo atlanır / video doğrudan Bunny'ye yüklenir / set otomatik işlenir) yerine üreticiye dönük sade ifade — *"Video ve soru setinizi yükledikten sonra yayın yönetimi aşamasındaki işlemler sonrası yayına açabilirsiniz."* ([`YeniTalepForm.tsx`](app/talepler/_components/YeniTalepForm.tsx)).
  - **İkili seçimde Açıklama + Ek Dosya girişi kapatıldı:** hazır video **ve** hazır soru seti birlikte seçiliyken Açıklama alanı ve "Dosya Ekle" butonu disable edilir (yalnız yeni giriş engellenir; mevcut state korunur). `EkDosyaYukleme`'ye `disabled` prop'u eklendi ([`YeniTalepForm.tsx`](app/talepler/_components/YeniTalepForm.tsx), [`EkDosyaYukleme.tsx`](app/talepler/_components/EkDosyaYukleme.tsx)).
  - tsc/denetim/lint temiz; DB yazımı yok.
