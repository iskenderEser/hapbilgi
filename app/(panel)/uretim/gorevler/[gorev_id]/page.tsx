...
export default function UretimGorevDetayPage() {
  ...
  const podcastYukle = async () => {
    if (!gorev || !podcastDosyalari.ses || !podcastDosyalari.transkript) return;
    setIslem(true);
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-4 px-3 py-4 md:px-6 md:py-6">
        ...
        <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
          ...
          <div className="flex flex-col gap-4 px-4 py-4 md:px-5">
            ...
            {iuTeslimEdebilir && gorev.asama === "video" && gorev.talep?.ogrenme_araci_turu === "podcast" && <div className="flex flex-col gap-3 border-t border-gray-100 pt-4"><label className="rounded-lg border border-dashed border-gray-300 p-3 text-sm">Ses dosyası<input type="file" accept="audio/*" className="mt-2 block w-full text-xs" onChange={(e) => setPodcastDosyalari((d) => ({ ...d, ses: e.target.files?.[0] }))} /></label><label className="rounded-lg border border-dashed border-gray-300 p-3 text-sm">Yayın görseli (isteğe bağlı)<input type="file" accept="image/jpeg,image/png,image/webp" className="mt-2 block w-full text-xs" onChange={(e) => setPodcastDosyalari((d) => ({ ...d, kapak: e.target.files?.[0] }))} /></label><label className="rounded-lg border border-dashed border-gray-300 p-3 text-sm">Transkript<input type="file" accept=".pdf,.txt,.doc,.docx" className="mt-2 block w-full text-xs" onChange={(e) => setPodcastDosyalari((d) => ({ ...d, transkript: e.target.files?.[0] }))} /></label><button type="button" onClick={() => void podcastYukle()} disabled={islem || !podcastDosyalari.ses || !podcastDosyalari.transkript} className="self-end rounded-lg border-0 bg-[#56aeff] px-5 py-2.5 text-xs font-semibold text-white disabled:opacity-50">İncelemeye Gönder</button></div>}
            ...
            {iuTeslimEdebilir && gorev.asama === "video" && gorev.talep?.ogrenme_araci_turu === "flip_pdf" && <div className="flex flex-col gap-3 border-t border-gray-100 pt-4"><label className="rounded-lg border border-dashed border-gray-300 p-3 text-sm">Nihai Literatür<input type="file" accept=".pdf,application/pdf" className="mt-2 block w-full text-xs" onChange={(e) => setFlipPdfDosyasi(e.target.files?.[0] ?? null)} /></label><button type="button" onClick={() => void flipPdfYukle()} disabled={islem || !flipPdfDosyasi} className="self-end rounded-lg border-0 bg-[#56aeff] px-5 py-2.5 text-xs font-semibold text-white disabled:opacity-50">İncelemeye Gönder</button></div>}
