...
export default function EczanemDagitimPage() {
  ...
  const videoDagit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        hata(data.hata ?? "Öğrenme içeriği gönderilemedi.", "öğrenme içeriği dağıtımı");
        return;
      }
      basari(data.mesaj ?? "Öğrenme içeriği müşterilere gönderildi.");
      setSeciliUyeler(new Set());
      await dagitimCek(true, seciliVideoId, true);
      bildirimRozetleriniYenile();
    } catch {
      hata("Öğrenme içeriği gönderilemedi.", "öğrenme içeriği dağıtımı");
    } finally {
    ...
    <div className="min-h-full bg-gray-50 pb-20 md:pb-8" style={{ fontFamily: "'Nunito', sans-serif" }}>
      <div className="mx-auto flex max-w-[1480px] flex-col gap-5 px-3 py-4 md:px-6 md:py-5 lg:px-8 lg:py-7">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-5 text-[#6b7f9b]">Eczanenize gelen öğrenme içeriklerini inceleyin ve aktif müşterilerinize tek işlemle gönderin.</p>
          </div>
          ...
          <>
            ...
            <section className="overflow-visible rounded-2xl border border-[#dfe7f1] bg-white shadow-[0_6px_18px_rgba(31,55,90,0.035)]">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#e5ecf4] px-4 py-3.5">
                <div>
                  <h2 className="text-base font-extrabold text-[#203653]">Müşterilere Gönderilecek Öğrenme İçerikleri</h2>
                  <p className="mt-0.5 text-[11px] font-semibold text-[#7b8da5]">{veri?.videolar.length ?? 0} içerik gösteriliyor</p>
                </div>
              ...
              {(veri?.videolar.length ?? 0) === 0 ? (
                <div className="px-4 py-14 text-center text-sm font-semibold text-[#8090a4]">Eczanenize gönderilmiş, dağıtıma hazır öğrenme içeriği bulunmuyor.</div>
      ...
      <AlertDialog open={onayAcik} onOpenChange={setOnayAcik}>
        <AlertDialogContent className="border-[#dbe5ef] bg-white">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[#203653]">Öğrenme içeriğini müşterilere gönderelim mi?</AlertDialogTitle>
            <AlertDialogDescription className="leading-6 text-[#687b90]">
              <strong className="text-[#30475f]">{seciliVideo?.urun_adi ?? "Seçili içerik"}</strong>, seçtiğiniz <strong className="text-[#30475f]">{seciliUyeler.size} müşterinin</strong> Eczanem öğrenme listesine eklenecek. Daha önce gönderilmiş kayıtlar tekrar oluşturulmaz.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
