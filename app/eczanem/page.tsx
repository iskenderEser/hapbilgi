...
export default function EczanemPanelPage() {
  ...
  const videolariCek = useCallback(async (elle = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setVideoHatasi(mesaj); hata(mesaj, data.adim ?? "videolar"); return;
      }
      setVideolar(data.videolar ?? []); setVideoHazir(true); setVideoHatasi(null);
    } catch (error) {
      if (!(error instanceof DOMException && error.name === "AbortError")) {
        setVideoHatasi("Videolar yüklenemedi."); hata("Videolar yüklenemedi.", "videolar");
      }
    } finally {
  ...
  const etkilesimDegistir = async (video: EczanemMusteriVideo, tur: "begeni" | "favori") => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { hata(data.hata ?? "Video etkileşimi kaydedilemedi.", data.adim ?? "video etkileşimi"); return; }
    ...
    } catch { hata("Video etkileşimi kaydedilemedi.", "video etkileşimi"); }
