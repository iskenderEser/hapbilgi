...
export default function VideolarListePage() {
  ...
  const veriCek = async () => {
    ...
    if (sdError) {
      hata("Videolar yüklenemedi.", "senaryo_durumu tablosu SELECT", sdError.message);
      setLoading(false);
      return;
    }
    ...
    for (const sd of senaryoDurumlari ?? []) {
      if (!benzersizMap.has(sd.senaryo_id)) {
        benzersizMap.set(sd.senaryo_id, sd.senaryo_durum_id);
      }
    }
    ...
    const sonuc = await Promise.all(
      Array.from(benzersizMap.entries()).map(async ([senaryo_id, senaryo_durum_id]) => {
        const { data: videolar } = await supabase
          .from("videolar")
          .select("video_id, video_url, thumbnail_url, created_at")
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
              ...
              <tbody>
                {filtreliSatirlar.map((v) => {
                  ...
                  return (
                    <tr key={v.senaryo_durum_id} onClick={() => router.push(`/videolar/${v.senaryo_durum_id}`)} style={{ borderBottom: "0.5px solid #f3f4f6", cursor: "pointer" }}>
                      <td style={{ padding: "12px 20px", color: "#111", fontWeight: 500 }}>{v.urun_adi}</td>
                      <td style={{ padding: "12px", color: "#737373" }}>{v.teknik_adi}</td>
                      ...
                      <td style={{ padding: "12px", color: "#737373", fontSize: "12px" }}>{formatTarih(v.son_tarih)}</td>
