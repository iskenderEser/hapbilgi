// app/analiz/api/uretici/sorgu/route.ts
//
// Üretici rolü (PM/egt_*/med_md) için analiz sayfasının "sorgu" endpoint'i.
...
export async function POST(request: NextRequest) {
  try {
    ...
    let analiz;
    try {
      analiz = await getUreticiAnalizData(user.id, filt);
    } catch (err) {
      return hataYaniti(
        "Analiz verisi çekilirken hata oluştu.",
        "getUreticiAnalizData",
    ...
    return sunucuHatasi(err, "POST /analiz/api/uretici/sorgu");
  }
}