-- scripts/sql/get_analiz_tm_tuketim.sql
--
-- Analiz sayfası — TM kolu tüketim metrikleri.
--
-- GÜNCELLEME (05.08.2026 — künye geçişi): BM kolundaki (get_analiz_bm_tuketim)
-- düzeltmenin aynısı. Yayından talebe giden sekiz tablolu SENARYO zinciri
-- gövdede yedi kez tekrarlanıyordu; hazır video kolunda senaryo yazılmadığı
-- için o yol kopuyordu (altı blokta süzgeç uygulandığında içerik düşüyor,
-- "izlenmeyen video" bloğunda hiç görünmüyordu). Yedi blokta da zincir
-- `v_yayin_kunye` ile değiştirildi; ürün süzgeci de defterdeki kopyadan değil
-- künyeden okunuyor.
--
...
BEGIN
  SELECT k.firma_id, k.takim_id INTO v_firma_id, v_takim_id
  FROM kullanicilar k
  ...
  END IF;
  IF v_takim_id IS NULL THEN
    RAISE EXCEPTION 'TM kullanıcısının takim_id tanımsız: %', p_kullanici_id;
  END IF;
