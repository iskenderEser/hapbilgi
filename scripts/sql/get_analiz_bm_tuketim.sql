-- scripts/sql/get_analiz_bm_tuketim.sql
--
-- Analiz sayfası — BM kolu tüketim metrikleri.
--
-- GÜNCELLEME (05.08.2026 — künye geçişi):
--   Fonksiyon yayından talebe SENARYO üzerinden gidiyordu; aynı sekiz tablolu
--   zincir gövdede yedi kez tekrarlanmıştı. Hazır video kolunda senaryo hiç
--   yazılmadığı için bu yol kopuyordu:
--     * Altı blokta bağ LEFT olduğundan satır düşmüyor ama talep bilgisi boş
--       geliyordu → kullanıcı ürün ya da eğitim türü süzgeci seçtiğinde hazır
--       video içeriği sonuçtan düşüyordu.
--     * "İzlenmeyen video" bloğunda bağ INNER olduğundan hazır video yayınları
--       o sayıma hiç girmiyordu — süzgeç olsun olmasın.
--   Yedi blokta da zincir `v_yayin_kunye` ile değiştirildi. Künye yayından
--   talebe doğrudan bağlanır (senaryo ve video halkaları yok), tek kaynaktır.
--
--   Ürün süzgeci de artık defterdeki kopyadan değil künyeden okunuyor; böylece
--   fonksiyon içindeki karışık kullanım (bazı bloklarda `kp.urun_id`, bazılarında
--   `t.urun_id`) tekleşti.
--
...
BEGIN
  ...
  END IF;
  IF v_takim_id IS NULL OR v_bolge_id IS NULL THEN
    RAISE EXCEPTION 'BM kullanıcısının takim_id veya bolge_id tanımsız: %', p_kullanici_id;
  END IF;

  -- KAZANIM 1: kazanilan_puanlar
  SELECT
  ...
  WHERE k.rol IN ('utt','kd_utt')
    ...
    AND k.takim_id = v_takim_id
    AND k.bolge_id = v_bolge_id
