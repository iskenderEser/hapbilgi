-- Öğrenme Araçları Genişletmesi — Faz 5 / Flip PDF üretim doğrulaması.
BEGIN;
SELECT pg_advisory_xact_lock(hashtextextended('hapbilgi-faz5-flip-pdf-uretim-v1', 1));

CREATE OR REPLACE FUNCTION public.uretim_flip_pdf_dogrula(
  p_arac_id uuid, p_kullanici_id uuid, p_gorev_id uuid,
  p_sayfa_sayisi integer, p_islem_anahtari uuid
)
RETURNS jsonb LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path TO 'public'
AS $fonksiyon$
DECLARE
  v_arac public.ogrenme_araclari%ROWTYPE; v_talep public.talepler%ROWTYPE;
  v_gorev public.uretim_gorevleri%ROWTYPE; v_durum_id uuid;
  v_sonraki jsonb := NULL; v_sonuc jsonb; v_onceki jsonb;
BEGIN
  IF p_islem_anahtari IS NULL OR p_sayfa_sayisi <= 0 THEN RAISE EXCEPTION 'İşlem anahtarı ve pozitif sayfa sayısı zorunludur.' USING ERRCODE = '22023'; END IF;
  ...
  IF v_arac.kaynak = 'iu' THEN
    ...
    IF NOT FOUND OR v_gorev.talep_id <> v_arac.talep_id OR v_gorev.asama <> 'video' OR v_gorev.atanan_iu_id IS DISTINCT FROM p_kullanici_id OR v_gorev.durum NOT IN ('hazirlaniyor', 'revizyon_bekliyor') THEN RAISE EXCEPTION 'Flip PDF üretim görevi geçersiz.' USING ERRCODE = '42501'; END IF;
    ...
    INSERT INTO public.ogrenme_araci_durumu (arac_id, durum, degistiren_id, notlar) VALUES (p_arac_id, 'inceleme bekleniyor', p_kullanici_id, 'Flip PDF üretici incelemesine gönderildi') RETURNING arac_durum_id INTO v_durum_id;
  ELSE
    IF v_talep.uretici_id IS DISTINCT FROM p_kullanici_id OR v_talep.hazir_video IS DISTINCT FROM true THEN RAISE EXCEPTION 'Hazır Flip PDF dosyasını yalnız talebin üreticisi tamamlayabilir.' USING ERRCODE = '42501'; END IF;
    INSERT INTO public.ogrenme_araci_durumu (arac_id, durum, degistiren_id, notlar) VALUES (p_arac_id, 'onaylandi', p_kullanici_id, 'Hazır Flip PDF — otomatik onay') RETURNING arac_durum_id INTO v_durum_id;
  ...
  INSERT INTO public.uretim_islem_kayitlari (islem_anahtari, islem_turu, gorev_id, talep_id, sonuc) VALUES (p_islem_anahtari, 'flip_pdf_dogrula', p_gorev_id, v_arac.talep_id, v_sonuc);
  RETURN v_sonuc;
END;
$fonksiyon$;
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended(p_islem_anahtari::text, 1));
  SELECT sonuc INTO v_onceki FROM public.uretim_islem_kayitlari WHERE islem_anahtari = p_islem_anahtari AND islem_turu = 'flip_pdf_uretici_karari';
  IF FOUND THEN RETURN v_onceki; END IF;
  SELECT * INTO v_gorev FROM public.uretim_gorevleri WHERE gorev_id = p_gorev_id FOR UPDATE;
  ...
  IF v_talep.uretici_id IS DISTINCT FROM p_uretici_id OR v_talep.ogrenme_araci_turu <> 'flip_pdf' THEN RAISE EXCEPTION 'Flip PDF karar yetkisi yok.' USING ERRCODE = '42501'; END IF;
  ...
  ELSIF v_gorev.asama = 'video' THEN
    ...
    IF NOT FOUND OR v_arac.arac_turu <> 'flip_pdf' OR v_arac.metadata_dogrulandi IS NOT TRUE OR v_arac.sayfa_sayisi <= 0 THEN RAISE EXCEPTION 'Doğrulanmış Flip PDF bulunamadı.' USING ERRCODE = '23514'; END IF;
  ...
  ELSE RAISE EXCEPTION 'Bu RPC yalnız Flip PDF senaryo ve üretim aşamasını işler.' USING ERRCODE = '23514'; END IF;
  ...
  INSERT INTO public.uretim_islem_kayitlari (islem_anahtari, islem_turu, gorev_id, talep_id, sonuc) VALUES (p_islem_anahtari, 'flip_pdf_uretici_karari', p_gorev_id, v_gorev.talep_id, v_sonuc);
  RETURN v_sonuc;
END;
$fonksiyon$;

REVOKE ALL ON FUNCTION public.uretim_flip_pdf_dogrula(uuid,uuid,uuid,integer,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.uretim_flip_pdf_dogrula(uuid,uuid,uuid,integer,uuid) TO service_role;
REVOKE ALL ON FUNCTION public.uretim_flip_pdf_uretici_karar_ver(uuid,uuid,text,text,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.uretim_flip_pdf_uretici_karar_ver(uuid,uuid,text,text,uuid) TO service_role;
COMMIT;
SELECT to_regprocedure('public.uretim_flip_pdf_dogrula(uuid,uuid,uuid,integer,uuid)') IS NOT NULL AS flip_pdf_dogrulama_kuruldu;
