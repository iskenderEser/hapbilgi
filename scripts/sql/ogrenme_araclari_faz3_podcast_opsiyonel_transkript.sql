-- ============================================================================
-- Podcast Opsiyonel Transkript RPC Güncellemesi
-- ============================================================================
-- Bu script, uretim_podcast_dogrula fonksiyonunu transkripti (transkript_yolu)
-- opsiyonel hale getirecek şekilde günceller.
-- Ses dosyası zorunluluğu ve pozitif süre kuralı korunur.
-- Transkript eklendiyse (dosya veya metin) doğrulanmış olması şartı aranır;
-- transkript eklenmediyse podcastin doğrulanması engellenmez.
-- ============================================================================
...
BEGIN
  ...
  -- Ses dosyası zorunludur
  IF v_arac.dosya_yolu IS NULL THEN
    RAISE EXCEPTION 'Ses dosyası doğrulanmadan podcast tamamlanamaz.' USING ERRCODE = '23514';
  END IF;

  -- İÜ podcast akışında (V1/V3) ses ve transkript dosyası zorunludur; dosya yolu olmadan onaylı metin kabul edilmez
  IF v_arac.kaynak = 'iu' THEN
    IF v_arac.transkript_yolu IS NULL
       OR COALESCE((v_arac.metadata->>'transkript_dogrulandi')::boolean, false) IS NOT TRUE THEN
      RAISE EXCEPTION 'İçerik üreticisi podcastinde ses ve transkript doğrulanmadan tamamlanamaz.' USING ERRCODE = '23514';
    END IF;
  ELSE
    -- Hazır podcast (kaynak = 'hazir'): Transkript opsiyoneldir; ancak transkript eklenmişse doğrulanmış olmalıdır
    IF (v_arac.transkript_yolu IS NOT NULL
        OR (v_arac.metadata->'transkript'->>'onaylanan_metin') IS NOT NULL
        OR (v_arac.metadata->>'transkript_metni') IS NOT NULL)
       AND COALESCE((v_arac.metadata->>'transkript_dogrulandi')::boolean, false) IS NOT TRUE
       AND COALESCE(v_arac.metadata->'transkript'->>'durum', '') <> 'onaylandi' THEN
  ...
  ELSE
    ...
    -- Kullanıcı AI transkript seçmişse zincir hemen açılmaz; kullanıcının onay veya iptal kararına bırakılır
    IF COALESCE(v_arac.metadata->'transkript'->>'durum', '') IN ('ai_bekliyor', 'ai_isleniyor', 'ai_taslak') THEN
      v_sonraki := NULL;
    ELSE
