-- scripts/sql/get_eclub_ligi_detay_aylik.sql
--
-- E-Club Ligi — bir UTT'nin dış müşterilerinin AYLIK ürün bazlı dökümü.
--
-- GÜNCELLEME (05.08.2026 — künye geçişi): Ürün kimliği üç ayrı yerden farklı
-- yollarla çözülüyordu — iki CTE puan defterindeki kopyadan (`kp.urun_id`,
-- `dc.urun_id`), biri satır satır `get_urun_from_yayin()` çağırarak. Üçü de
-- yayın künyesine (`v_yayin_kunye`) bağlandı: tek kaynak, tek yol.
--
-- Yan kazanım: `video` CTE'sindeki satır-başına fonksiyon çağrısı kalktı
-- (GROUP BY içinde de tekrarlanıyordu); yerine tek birleştirme geldi.
--
-- `LEFT JOIN urunler` bilinçli olarak LEFT'tir — ürünsüz içerik satırı düşmez,
-- yalnız ürün adı boş görünür.
--
...
CREATE OR REPLACE FUNCTION public.get_eclub_ligi_detay_aylik(p_utt_id uuid, p_yil integer, p_ay integer)
...
AS $function$
  WITH d AS (
    SELECT
      make_timestamptz(p_yil, p_ay, 1, 0, 0, 0) AS bas,
      (make_timestamptz(p_yil, p_ay, 1, 0, 0, 0) + interval '1 month') AS son
  ),
  -- p_utt_id'nin gönderdiği, izlenen öneriler → kişi + ürün bazında
  temel AS (
    SELECT
      o.kisi_id,
      ky.urun_id,
    ...
    WHERE o.oneren_id = p_utt_id
      AND kp.created_at >= d.bas AND kp.created_at < d.son
  ...
  dogru AS (
    ...
    WHERE o.oneren_id = p_utt_id
      AND dc.created_at >= d.bas AND dc.created_at < d.son
    GROUP BY o.kisi_id, ky.urun_id
  ),
  -- kisi_id birleşimi (temel + video + dogru farklı kişi/ürünler içerebilir)
  anahtar AS (
  ...
  SELECT
    ecz.gln,
    m.eczane_adi,
    eczaci.ad_soyad AS eczaci_ad,
    tekn.ad_soyad AS teknisyen_ad,
    a.urun_id,
    ur.urun_adi,
    COALESCE(t.izleme_puani, 0),
    COALESCE(t.cevaplama_puani, 0),
    COALESCE(v.izlenen_video, 0),
    COALESCE(dg.dogru_cevap, 0)
  FROM anahtar a
  ...
  LEFT JOIN LATERAL (
    SELECT (kk.ad || ' ' || kk.soyad) AS ad_soyad
    FROM eclub_kisi_eczane kke
