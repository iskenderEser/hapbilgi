-- scripts/sql/get_eclub_ligi_detay_yillik.sql
--
-- E-Club Ligi — bir UTT'nin dış müşterilerinin YILLIK ürün dökümü.
-- Aylık ve dönemlik ikizlerinin yıl penceresi.
--
...
CREATE OR REPLACE FUNCTION public.get_eclub_ligi_detay_yillik(p_utt_id uuid, p_yil integer)
...
AS $function$
  WITH d AS (
    SELECT make_timestamptz(p_yil, 1, 1, 0, 0, 0) AS bas,
      (make_timestamptz(p_yil, 1, 1, 0, 0, 0) + interval '1 year') AS son
  ),
  temel AS (
